...
# --- LIVE DATA MAILBOX ---
# This holds the real-time data sent by the laptop agent
live_data_store = {
    "risk_score": 0.0,
    "recent_packets": []
}
...
@app.route('/live')
def live_traffic():
    """Renders the real-time monitoring page."""
    return render_template('live.html', active_page='live')

@app.route('/logs')
def logs():
    """Renders the historical logs page."""
...
def push_live():
    """API Endpoint: The Laptop Agent secretly sends CSV data here every few seconds."""
    ...
    try:
        ...
        risk = round((attacks / total) * 100, 1) if total > 0 else 0
        
        # Create a log for the terminal (last 10 packets)
        packet_logs = []
        for i in range(min(10, total)):
            packet_logs.append({
                "id": i + 1,
                "is_attack": int(predictions[i])
            })
            
        # Update the Mailbox
        live_data_store = {
            "risk_score": risk,
            "recent_packets": packet_logs
        }
...
@app.route('/api/get-live', methods=['GET'])
def get_live():
    """API Endpoint: The HTML Dashboard asks this route for the latest data."""
